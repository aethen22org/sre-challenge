...
#	@docker pull ${DOCKER_REGISTRY}/${IMAGE_NAME}:latest --cache-from ${DOCKER_REGISTRY}/${IMAGE_NAME}:latest 
	@echo "Pulled image"
	@echo "Building image"
	@docker build . --build-arg BUILDKIT_INLINE_CACHE=1 -t ${DOCKER_REGISTRY}/${IMAGE_NAME}:${COMMIT_SHA} -t ${DOCKER_REGISTRY}/${IMAGE_NAME}:latest
	@echo "Built image"
ifdef GIT_TAG
